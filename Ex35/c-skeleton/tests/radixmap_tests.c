...
static char *test_operations(){
    ...
    while(map->end > 0){
        RMElement *value = RadixMap_find(map, map->contents[map->end / 2].data.key);
        mu_assert(value != NULL, "Should get a result.");
        
        size_t old_end = map->end;

        mu_assert(RadixMap_delete(map, value) == 0, "Didn't delete it.");
