...
static inline void radix_sort(short offset, uint64_t max, uint64_t *source, uint64_t *dest){
    ...
    //elements and storing in the same array
...
int RadixMap_delete(RadixMap *map, RMElement *value){
    check(map->end > 0, "There is nothing to delete.");
    check(value != NULL, "Can't delete a NULL element.");

    value->data.key = UINT32_MAX;
