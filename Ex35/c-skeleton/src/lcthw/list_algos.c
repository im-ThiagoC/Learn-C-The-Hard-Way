...
void *List_insert(List *list, ListNode *node, void *value){
    check(list != NULL, "List can't be null to insert");
    check(node != NULL, "Node can't be null to insert");
    check(value != NULL, "Value can't be null to insert");
...
void List_bottom_up_merge(List *list, int left, int right, int end, List *sorted_List, List_compare cmp) {
    ...
    while (i < right || j < end) {
        ...
        if(i < right && cmp(leftNode->value, rightNode->value) < 0){
